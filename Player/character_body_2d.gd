...
var impala_phrase_index: int = 0   # індекс для чергування фраз

func _ready() -> void:
	var hp_bar = get_node_or_null("/root/Node2D/CanvasLayer3/ProgressBar")
	if hp_bar:
...
func _on_impala_interacted():
	if dialogue_label:
		...
		print("Взаємодія з Impala спрацювала!")

		# таймер для очищення тексту через 3 секунди
		var timer = Timer.new()
		timer.wait_time = 9.0
		timer.one_shot = true
		add_child(timer)
